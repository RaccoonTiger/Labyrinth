...
public class PlanDeJeu extends MonObservable implements MonObserver, Runnable {
	...
	private final Vector<AbstractCreature> creatures = new Vector<AbstractCreature>(10);
	private final Vector<AbstractEquipement> equipements = new Vector<AbstractEquipement>(10);
	private final Random rand = new Random(System.currentTimeMillis());
	public static ArrayList<String> messagesPanneauBas = new ArrayList<String>();
	...
	private void validerEtatJeu(){
		...
		if(!gestCombat.combatEstEnCours()){
			...
			for(int i=0;i<this.equipements.size();i++){
				AbstractEquipement item = this.equipements.get(i);

				if(this.joueur.itemDetecte(item) && item.estAusol()){
					this.joueur.ramasser(item);
					item.setAuSol(false);
					System.out.println("item get: " + item);
					messagesPanneauBas.add("item get: " + item);
				}
			}
	...
	private void partieGagne(){
		...
		niveauCourant++;
		messagesPanneauBas.add("Fin de niveau");
